...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({ name, title, description, imageSrc }) => {
  return (
    <header className="flex flex-col md:flex-row items-center md:items-start gap-6 text-center md:text-left">
      ...
      <div className="space-y-2">
        <h1 className="text-3xl md:text-4xl font-bold hover:text-primary transition-colors duration-300">{name}</h1>
        <p className="text-xl text-gradient font-semibold glow-text">{title}</p>
